...
void input_8_bytes(ifstream& inFile, DES_DataType& plainText)
{
	for (int i = 7; i >= 0; i--) {
		inFile >> noskipws >> plainText.array2[i];
	}
}
void output_8_bytes(ofstream& OutFile, DES_DataType& plainText)
{
	for (int i = 7; i >= 0; i--) {
		OutFile << plainText.array2[i];
	}
}
void input_hex(ifstream& inFile, DES_DataType& plainText)
{
	string x;
	for (int i = 7; i >= 0; i--) {
		inFile >> hex >> setfill('0') >> setw(2) >> uppercase >> x;

		try
		{
			plainText.array2[i] = stoi(x, 0, 16);
...
void output_hex(ofstream& OutFile, DES_DataType& plainText)
{
	for (int i = 7; i >= 0; i--) {
		OutFile << int_to_hex((int)plainText.array2[i]);
	}
...
int main(void) {
	...
	if (false) {
		// Encrypt
		string DataPath = "./plain_text.txt";
		string OutPathTXT = "./Encrypted.txt";
		string OutPathHEX = "./HEX.txt";
		string KeyPath = "./Key.txt";

		f = fopen(DataPath.c_str(), "rb");
		fout = fopen(OutPathTXT.c_str(), "wb");
		fhexout = fopen(OutPathHEX.c_str(), "wb");

		ifstream KeyFile(KeyPath), inFile(DataPath);
		ofstream OutFile(OutPathTXT);
		ofstream OutHexFile(OutPathHEX);

		OutFile.rdbuf()->pubsetbuf(buffer, BUFFER_SIZE);
		OutHexFile.rdbuf()->pubsetbuf(bufferHEX, BUFFER_SIZE);

		DES_KeyType key;
		KeyFile >> hex >> key.value_64;
		cout << "Key is 0x" << hex << uppercase << key.value_64 << endl;
		...
		while (inFile) {
			...
			input_8_bytes(inFile, plainText);
			PerformOperation(plainText, key, ENCRYPT);

			output_8_bytes(OutFile, plainText);
			output_hex(OutHexFile, plainText);
		}
		KeyFile.close();
		inFile.close();
		OutFile.close();
	...
	{
		// Decrypt
		string DataPath = "./HEX.txt";
		string OutPathTXT = "./Decrypted.txt";
